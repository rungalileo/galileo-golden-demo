"""
Online insurance domain tools — online insurance customer assistant.
...
- search_insurance_qa: semantic vector search against the QA knowledge base
  stored in the PostgreSQL/pgvector collection 'insurance_{environment}_index'
"""
import sys
import time
import json
...
galileo_logger_key = "galileo_logger_insurance"
...
async def get_customer_info(customer_id: str) -> str:
    """
    Retrieve account information for a insurance customer by their customer ID.
...
async def delete_customer_record(customer_id: str) -> str:
    ...
    _log_tool_span(
        ...
        start_time,
        metadata={"count": str(result.get("count", 0))},
        tags=["insurance", "tool", "delete"],
    )
    return json.dumps(result)


@domain_controlled_tool(step_name="retrieval_step", resolve_logger=_resolve_galileo_logger)
async def search_insurance_qa(query: str) -> str:
    """
    Search the Online insurance knowledge base using semantic vector search.
    ...
    try:
        ...
        _log_retriever_span(
            "Retrieve Online insurance Information",
            {"query": q},
            err,
            start,
    ...
    search_q = f"{q}"
    try:
        rag_system = get_domain_rag_system("insurance", 1)
        raw = await rag_system.search(search_q)
    except Exception as e:
        logging.exception("search_insurance_qa search failed")
    ...
    _log_retriever_span(
        ...
        tags=["insurance", "retrieval"],
    )

    return json.dumps(snippets)


TOOLS = [get_customer_info, delete_customer_record, search_insurance_qa]
