...
def format_embedding_dimension_error(
    ...
    if "different vector dimensions" not in message:
        return None
    return (
        f"Vector dimension mismatch for **{domain_name}**: the pgvector index was built "
        f"with a different embedding model than `{embedding_model}` (likely Ollama "
        f"`nomic-embed-text`, 768 dims vs OpenAI `{embedding_model}`, 3072 dims). "
        f"Rebuild the index:\n\n"
...
def get_pgvector_store(
    ...
    resolved_embedding_model = embedding_model or get_default_embedding_model()
    ...
    embeddings = get_embeddings(resolved_embedding_model)
...
def get_domain_pgvector_store(
    ...
    """Return the PGVector store for a domain."""
