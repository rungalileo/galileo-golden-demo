...
def load_relational_csv(
    ...
    Drops and recreates the table, inserts all rows, and adds a primary key
    plus indexes on non-PK text columns commonly used in WHERE clauses.
    """
    ...
    with engine.begin() as conn:
        ...
        for col in columns:
            if col == pk_col:
                continue
            conn.execute(
                text(f'CREATE INDEX IF NOT EXISTS "{table_name}_{col}_idx" ON "{table_name}" ("{col}")')
            )
...
def load_domain_relational_csvs(docs_dir: str | Path, domain_name: str) -> List[Tuple[str, int]]:
    """Load every relational_*.csv file in a domain docs directory."""
...
def execute_sql(sql: str) -> Dict[str, Any]:
    ...
    SELECT statements return matching rows. DML statements (e.g. DELETE) are
    committed automatically via a transaction.
    """
