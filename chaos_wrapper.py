...
def wrap_tools_with_chaos(tools: list) -> list:
    """
    Wrap all tools with chaos injection - SIMPLIFIED!
    
    No complex pattern matching. Just wraps every tool the same way.
    This is a DEMO - chaos applies to everything!
    ...
    for tool in tools:
        ...
        def wrapper(*args, __original_func=tool, __display_name=display_name, __func_name=func_name, **kwargs):
            ...
            # 5. Data Corruption (applies to all tools - this is a demo!)
            if chaos.data_corruption_enabled:
                try:
                    if isinstance(result, str):
                        ...
                        result = chaos.corrupt_data(result)
                except (json.JSONDecodeError, TypeError, AttributeError):
                    pass  # Not JSON, skip corruption
...
def wrap_rag_tool_with_chaos(rag_tool):
    ...
    """
    chaos = get_chaos_engine()
    original_invoke = rag_tool.invoke if hasattr(rag_tool, 'invoke') else rag_tool
    
    def invoke_with_chaos(*args, **kwargs):
        # Check if RAG should fail THIS query
    ...
    # Replace invoke method if it's a LangChain tool
    if hasattr(rag_tool, 'invoke'):
        rag_tool.invoke = invoke_with_chaos
        return rag_tool
    else:
        # It's a function, return wrapped function
        return invoke_with_chaos
